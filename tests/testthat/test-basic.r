...
test_that("sd, skew, kurt run without error",{#FOLDUP
	...
	for (na_rm in c(FALSE,TRUE)) {
		...
		std_cumulants(x,max_order=5L,used_df=1L,na_rm=na_rm)
		...
		std_cumulants(y,max_order=5L,used_df=1L,na_rm=na_rm)
		...
		std_cumulants(z,max_order=5L,used_df=1L,na_rm=na_rm)
	}
